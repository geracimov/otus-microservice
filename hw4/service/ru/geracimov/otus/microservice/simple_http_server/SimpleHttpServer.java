package ru.geracimov.otus.microservice.simple_http_server;
...
public class SimpleHttpServer {
    private static final String LOG_EMPLATE = "[%1$tF %1$tT] [%4$-7s] %5$s%n";
    ...
    static {
        System.setProperty("java.util.logging.SimpleFormatter.format", LOG_EMPLATE);
    ...
    public static void main(String[] args) throws IOException {
        ...
        server.createContext("/hello", httpExchange -> {
            log(httpExchange);
            String body = "SimpleHttpServer v.2 from " + InetAddress.getLocalHost();
            response(httpExchange, TEXT_HTML_CONTEXT_TYPE, body);
        });
        server.createContext("/health", httpExchange -> {
            log(httpExchange);
            var body = "{\"status\": \"OK\", \"version\": \"v2\"}";
